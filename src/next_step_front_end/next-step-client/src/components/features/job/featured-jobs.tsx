...
import { fetchFeaturedJobs, toggleFavoriteJob } from "@/store/slices/jobs-slice"
...
export default function FeaturedJobs({ filter }: { filter: JOB_FILTER_TYPES }) {
  ...
  const { featured, status } = useSelector((state: RootState) => state.jobs)
  ...
  if (status === "failed") return <AlertDestructive message="Failed to fetch featured jobs" />
    ...
    return (
      <div className="text-center py-20 animate-in fade-in-50 duration-500">
        ...
        <p className="text-muted-foreground max-w-md mx-auto leading-relaxed font-medium animate-in slide-in-from-bottom-4 duration-500 delay-500">
          We couldn't find any jobs matching your criteria. Try adjusting your filters or check back later for new
          opportunities.
        </p>
      </div>
    )
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {featured.map((job, index) => (
          ...
          >
              ...
              <div className="p-6 pb-4">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    <Avatar className="h-14 w-14 border-none rounded-xl p-0.5 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3">
                      <AvatarImage
                        src={job.companyLogo || ""}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center text-muted-foreground mb-2 transition-all duration-300 group-hover:text-foreground">
                        <Building2 className="h-4 w-4 mr-2 transition-transform duration-300 group-hover:scale-110" />
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {job.salary && (
                      <div className="flex items-center bg-secondary border px-3 py-2 rounded-full transition-all duration-300 hover:bg-accent hover:scale-105 hover:shadow-sm">
                        ...
                        <span className="text-xs text-muted-foreground ml-1 font-medium">.{job.salary.payPeriod}</span>
                      </div>
                    )}
            ...
            <CardFooter className="bg-muted/50 px-6 py-4 border-t transition-all duration-300 group-hover:bg-muted/70 flex items-center justify-between">
              ...
              >
                <span className="transition-all duration-300 group-hover:translate-x-1">View Details</span>
              </Button>
            </CardFooter>
          </Card>
        ))}
