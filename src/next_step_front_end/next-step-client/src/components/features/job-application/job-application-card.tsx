...
export default function JobApplicationCard() {
  ...
  // Redux state
  const { info: applicationInfo } = useSelector(
    (state: RootState) => state.jobApplications,
  );
  const { selected: selectedJob } = useSelector(
  ...
  const [resumeOption, setResumeOption] = useState<"existing" | "upload">(
    "existing",
  );
    ...
    try {
      ...
      if (resumeOption === "upload" && uploadedFile) {
        ...
        try {
          ...
          showNotification(
            "error",
            "Resume Error",
            "Failed to load existing resume. Please upload a new one.",
          );
      ...
      if (!fileToSubmit) {
        showNotification(
          "error",
          "Resume Required",
          "Please select or upload a resume file.",
        );
        setIsSubmitting(false);
        return;
      }
  ...
  const isExpired =
    ...
    new Date(applicationInfo.expiryDate) < new Date();
              ...
              >
                <div className="bg-gradient-to-br from-muted/20 to-muted/10 p-4 rounded-xl border border-border/30 shadow-sm hover:shadow-md transition-all duration-300">
                  ...
                  <div className="text-2xl font-bold text-primary">
                    {applicationInfo.countJobApplications}
                  </div>
                  ...
                  >
                    ...
                    >
                      {isExpired
                        ? "Expired"
                        : formatDistanceToNow(
                            new Date(applicationInfo.expiryDate),
                            { addSuffix: true },
                          )}
                    </div>
                    <div className="text-xs text-muted-foreground">
        ...
        <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="text-base">
              Complete your application by providing a cover letter and
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-4">
              ...
              <RadioGroup
                value={resumeOption}
                onValueChange={(value: "existing" | "upload") =>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Resume already uploaded to your profile
                          </p>
                        </div>
                        <Button
                          variant="outline"
