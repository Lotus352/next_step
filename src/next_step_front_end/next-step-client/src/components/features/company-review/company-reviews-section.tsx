...
function CompanyReviewsSection() {
    ...
    const {content, statuses, totalElements} = useSelector((state: RootState) => state.companyReviews)
    ...
    const handleShowMore = () => {
        const remain = content.length - visibleCount
        if (remain > 0) {
            setVisibleCount((prev) => prev + (remain < 5 ? remain : 5))
        }
    }

    const filteredReviews = filterRating ? content.filter((review) => review.rating === filterRating) : content
    ...
    return (
        <>
            <Card
                className="border-primary/10 shadow-lg hover:shadow-xl transition-all duration-300 backdrop-blur-sm bg-white/95">
                <CardHeader
                    className="pb-4 border-b border-primary/10 bg-gradient-to-br from-primary/5 via-primary/3 to-transparent">
                ...
                <CardContent className="pt-6">
                    ...
                    {statuses.fetching  === "failed" && <DestructiveAlert message="Failed to fetch reviews"/>}

                    {statuses.fetching  === "succeeded" && (
                        <>
                            {!hasReviewed && user && (
                                <motion.div
                                ...
                                >
                                    ...
                                    <div className="space-y-5">
                                        <div>
                                            <label className="block text-sm font-semibold mb-3 text-primary/80">Your
                                                Rating</label>
                                            <ReviewStar
                                ...
                                <AnimatePresence>
                                    {visibleReviews.length > 0 ? (
                                        ...
                                        <div className="text-center py-12 px-4">
                                            <div
                                                className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-primary/10 to-primary/5 mb-6 shadow-lg">
