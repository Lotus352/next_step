...
import { User, Mail, Phone, FileText, Upload, CheckCircle2, Briefcase, GraduationCap, Flag } from "lucide-react"
import { uploadAvatar, uploadResume, fetchUserProfile } from "@/store/slices/user-slice"
...
export default function ProfileHeader() {
    ...
    const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            showNotification("loading", "Uploading Avatar", "Please wait while we upload your profile picture...")

            if (avatarFile) {
                dispatch(uploadAvatar({ id: profile.userId, file: avatarFile }))
                    .unwrap()
                    .then(() => {
                    ...
                    .catch((error) => {
                        ...
                        showNotification(
                            "error",
                            "Upload Failed",
                            error?.message || "There was a problem uploading your profile picture",
                        )
                        setAvatarFile(null)
                    })
            }
    ...
    const handleResumeUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            // Show loading notification
            showNotification("loading", "Uploading Resume", "Please wait while we upload your resume...")

            if (resumeFile) {
                dispatch(uploadResume({ id: profile.userId, file: resumeFile }))
                    .unwrap()
                    .then(() => {
                        // Show success notification
                        showNotification("success", "Resume Updated", "Your resume has been uploaded successfully")
                        setResumeFile(null)
                    })
                    .catch((error) => {
                        // Show error notification
                        showNotification("error", "Upload Failed", error?.message || "There was a problem uploading your resume")
                        setResumeFile(null)
                    })
            }
        }
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {profile.experiences
                                            ...
                                            .map((exp) => (
                                                <Badge
                                                    key={`${exp.userId}-${exp.experienceId}`} // Unique key using composite key
                                                    variant="outline"
                                                        ...
                                                        <TooltipTrigger asChild>
                                                            <label className="inline-flex h-8 px-3 items-center justify-center rounded-md border border-input bg-background hover:bg-accent hover:text-accent-foreground text-xs cursor-pointer">
                                                                <input
                                                                    ...
                                                                    disabled={statuses.uploadingResume === "loading"}
                                                                />
                                                                {statuses.uploadingResume === "loading" ? "Uploading..." : "Update"}
                                                            </label>
                                                        </TooltipTrigger>
                                                        <TooltipContent>
                                        ...
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <Button variant="outline" size="sm" className="gap-1" asChild>
                                                <label className="cursor-pointer">
                                                    ...
                                                    {statuses.uploadingResume === "loading" ? "Uploading..." : "Upload Resume"}
                                                </label>
                                            </Button>
                                        </div>
                                    )}
