...
import {Calendar, Users, CheckCircle, XCircle, TableIcon} from "lucide-react"
...
export function JobManagementTable(
            ...
            <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} className="text-center py-12">
                <Card className="border-border/30 bg-background/80 backdrop-blur-sm shadow-lg">
                    <CardContent className="p-12">
                        ...
                        <p className="text-muted-foreground text-sm">There are currently no job listings matching your
                            criteria.</p>
                    </CardContent>
                </Card>
            </motion.div>
    ...
    return (
        ...
        >
                                        ...
                                        >
                                            <TableCell className="py-4">
                                                <div className="space-y-1">
                                                    <div
                                                        className="font-semibold text-foreground group-hover:text-primary transition-colors duration-300 line-clamp-1">
                                                        {job.title}
                                                    </div>
                                                    <div
                                                        className="text-sm text-muted-foreground line-clamp-2">{job.shortDescription}</div>
                                                    {job.skills.length > 0 && (
                                            ...
                                            <TableCell className="py-4">
                                                <div className="flex items-center gap-3">
                                                    <Avatar className="h-8 w-8 rounded-lg">
                                                        <AvatarImage src={job.postedBy.company.logoUrl || ""}
                                                                     alt={job.postedBy.company.name}/>
                                                        <AvatarFallback
                                                            className="bg-primary/10 text-primary font-semibold text-xs rounded-lg">
                                                            {job.postedBy.company.name?.charAt(0) || "C"}
                                                        </AvatarFallback>
                                                    </Avatar>
                                                    <div>
                                                        <div
                                                            className="font-medium text-sm text-foreground">{job.postedBy.company.name}</div>
                                            ...
                                            <TableCell
                                                className="py-4">{getApplicationsBadge(job.appliedCount)}</TableCell>
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This action cannot be undone. This will permanently delete the job posting and remove all
                            associated data.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <JobEditDialog job={selectedJob} open={editDialogOpen} onOpenChange={setEditDialogOpen}
                           userRole={userRole}/>
        </motion.div>
    )
}
