...
export function UserManagementHeader({ onFilterToggle, showFilters, userRole }: UserManagementHeaderProps) {
    const dispatch = useDispatch<AppDispatch>()
    const { filter, statuses } = useSelector((state: RootState) => state.user)
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            dispatch(setUserFilter(updatedFilter))
            await dispatch(
                filterUsers({
                    page: DEFAULT_PAGE,
            ...
            {hasActiveFilters && (
                ...
                >
                    ...
                    {filter.isDeleted !== undefined && (
                        <Badge variant="secondary" className="bg-primary/10 text-primary border-primary/20">
                            Status: {filter.isDeleted ? "Deleted" : "Active"}
                        </Badge>
                    )}
                </motion.div>
            )}
