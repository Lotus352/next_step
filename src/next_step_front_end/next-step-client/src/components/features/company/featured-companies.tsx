...
import {fetchFeaturedCompanies} from "@/store/slices/companies-slice"
...
import {resetStatuses} from "@/store/slices/job-applications-slice.ts";

export default function FeaturedCompanies() {
    const dispatch = useDispatch<AppDispatch>()

    // Redux state
    const {featured, statuses} = useSelector((state: RootState) => state.companies)

    useEffect(() => {
        dispatch(fetchFeaturedCompanies(FEATURED_COMPANIES_LIMIT))
        return () => {
    ...
    if (statuses.fetchingFeatured === "failed") return <AlertDestructive message="Failed to fetch featured companies"/>
    ...
    return (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {featured.map((company, index) => (
                ...
                >
                    <Card
                        className="group overflow-hidden hover:shadow-lg transition-all duration-500 hover:-translate-y-2 hover:scale-[1.02] h-full">
                        <CardContent className="p-6">
                            <div className="flex flex-col items-center space-y-4">
                                {/* Company Avatar */}
                                <div className="relative">
                                    <Avatar
                                        className="h-16 w-16 border-none rounded-xl p-0.5 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3">
                                        <AvatarImage
                                            src={company.logoUrl || ""}
                                    ...
                                    {/* Verified Badge */}
                                    <div
                                        className="absolute -top-1 -right-1 h-6 w-6 bg-green-500 rounded-full flex items-center justify-center animate-pulse">
                                ...
                                <div className="space-y-3 text-center w-full">
                                    ...
                                    {/* Job Count */}
                                    <div
                                        className="flex items-center justify-center gap-2 text-muted-foreground transition-all duration-300 group-hover:text-foreground">
                                        <Users
                                            className="h-4 w-4 transition-all duration-300 group-hover:text-primary"/>
                                        <span
                                            className="text-sm font-medium">{company.countJobOpening} open positions</span>
                                    </div>
