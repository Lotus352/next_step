...
>("skills/fetch", async ({page = DEFAULT_PAGE, size = DEFAULT_LEVEL_SIZE, key = DEFAULT_KEYWORD} = {}) => {
...
export const deleteSkill = createAsyncThunk<number, number>("skills/delete", async (id, {rejectWithValue}) => {
    try {
...
export const {clearSkills, clearCurrentSkill, clearError, resetStatuses, setSearchKeyword} = skillsSlice.actions;
