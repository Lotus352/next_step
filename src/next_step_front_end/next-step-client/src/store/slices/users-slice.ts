...
>("user/changeStatus", async ({id, status}) => {
    const {data} = await axiosClient.put(`/api/user/${id}/status`, {status});
    return data;
});
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(changeUserStatus.pending, (state) => {
                state.statuses.changingStatus = "loading";
                state.error = null;
            })
            .addCase(changeUserStatus.fulfilled, (state, action) => {
                state.statuses.changingStatus = "succeeded";
                state.content = state.content.map((user) =>
                    user.userId === action.payload.userId ? { ...user, status: user.status } : user,
                );
            })
            .addCase(changeUserStatus.rejected, (state) => {
                state.statuses.changingStatus = "failed";
                state.error = "Failed to change user status";
            });
    },
});
